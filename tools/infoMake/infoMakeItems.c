...
int main(int argc, char** argv)
{
	...
	uint8_t compressedData[numPokes][32];
	...
	puts("//item info:");
	puts("// name length (5 bits, length = encoded + 4), then 7 bits per char");
	puts("// type (5 bits)");
	puts("HEX \"INFO\" ID 0");
	...
	printf("//Now: offsets into this resource for each item's data (16-bit Big-Endian)\n");
	printf("//offset for item N is stored as (actual_offset - 5 * N)\n");

	ofst = 0;
	for (i = 0; i < numPokes; i++) {
		uint32_t effectiveOffset = ofst - 5 * i;

		if (effectiveOffset > 0xFFFF) {
			fprintf(stderr, "offset not encodeable (exceeds 64KB)\n");
			abort();
		}

		// Emit 16-bit offset (Big-Endian)
		printf(" 0x%02x 0x%02x // offset for item %u (%u encoded)\n",
				(uint8_t)(effectiveOffset >> 8), (uint8_t)effectiveOffset, i + 1, effectiveOffset);
